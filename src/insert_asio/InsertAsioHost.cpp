...
namespace
{
constexpr int DefaultInsertSampleRate = 48000;
...
double chooseSampleRate(const juce::Array<double>& rates, int requested, int fallback) noexcept
{
    if (rates.isEmpty())
        return requested > 0 ? static_cast<double>(requested) : static_cast<double>(fallback);
    ...
    const auto best = std::min_element(begin, end, [requested, fallback](double left, double right) {
        const double target = requested > 0 ? static_cast<double>(requested) : static_cast<double>(fallback);
        return std::abs(left - target) < std::abs(right - target);
    });

    return best != end ? *best : static_cast<double>(fallback);
}
...
bool InsertAsioHost::start(int requestedSampleRate, int requestedBlockSize, int expectedChannelCount, std::wstring& status)
{
    ...
    const auto rate = chooseSampleRate(nextDevice->getAvailableSampleRates(), requestedSampleRate, DefaultInsertSampleRate);
    ...
    const juce::AudioIODeviceCallbackContext&)
{
    ...
    realtimeEngine.process(buffer, CallbackStreamKind::InputInsert);
